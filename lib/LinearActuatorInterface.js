class LinearActuatorInterface {
	...
	safetyModeSequence() {
		...
		if(this.sequence != 'safety') {
			console.log("Starting safety sequence");
			this.sequence = 'safety';
			
			var _this = this;
			
			//Extend the actuator for 15s
			_this.retract(30000, function() {
				console.log("Retraction done. Sending stop signal");
				//Set the stop signal at the bottom
				_this.stop(15000, function() {
					_this.sequence = 'none';
					console.log("Done sending stop signal");
					console.log("Sequence Completed w/ Sequence State: " + _this.sequence);
				});
			});
